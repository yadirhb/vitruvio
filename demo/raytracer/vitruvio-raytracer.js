...
Class('Camera', {
    constructor : function(pos, lookAt) {
    	this.pos = pos;
...
Class('Sphere', {
	...
	surface : Surface,
    radius2: Number,
    constructor : function(center, radius, surface) {
    	this.center = center;
    	this.radius = radius;
...
Class('Plane', {
    ...
    normal : function(pos){},
    intersect: function(ray){},
    constructor : function(norm, offset, surface) {
    	this.surface = surface;
...
(function(){
	...
	function intersections(ray, scene) {
        ...
        for (var i in scene.things) {
        	if(scene.things.hasOwnProperty(i)) {
    ...
    function testRay(ray, scene) {
        var isect = intersections.call(this, ray, scene);
        if (isect != null) {
            return isect.dist;
        } else {
            return undefined;
    ...
    function traceRay(ray, scene, depth){
        ...
        if (isect === undefined) {
            ...
            return shade.call(this, isect, scene, depth);
        }
    }
    ...
    function shade(isect, scene, depth) {
        ...
        var naturalColor = Color.plus(Color.background,
                                      getNaturalColor.call(this, isect.thing, pos, normal, reflectDir, scene));
        var reflectedColor = (depth >= this.maxDepth) ? Color.grey : getReflectionColor.call(this, isect.thing, pos, normal, reflectDir, scene, depth);
        return Color.plus(naturalColor, reflectedColor);
    }

    function getReflectionColor(thing, pos, normal, rd, scene, depth) {
        return Color.scale(thing.surface.reflect(pos), traceRay.call(this, { start: pos, dir: rd }, scene, depth + 1));
    }

    function getNaturalColor(thing, pos, norm, rd, scene) {
        function addLight(col, light) {
            var ldis = Vector.minus(light.pos, pos);
            var livec = Vector.norm(ldis);
            var neatIsect = testRay.call(this, { start: pos, dir: livec }, scene);
	    ...
	    render : function(scene, ctx, screenWidth, screenHeight) {
	        ...
	        for (var y = 0; y < screenHeight; y++) {
	            for (var x = 0; x < screenWidth; x++) {
	                var color = traceRay.call(this, { start: scene.camera.pos, dir: getPoint(x, y, scene.camera) }, scene, 0);
...
function exec() {
    ...
    var result = rayTracer.render(defaultScene(), ctx, canv.width, canv.height);

    console.log((new Date().getTime()) - _initTime);
    return result;
}
